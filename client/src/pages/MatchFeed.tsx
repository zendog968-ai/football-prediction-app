    ...
    <section className="mx-auto max-w-5xl px-4 pt-6 xl:px-8">
      ...
      {query.isLoading ? <div className="rounded-2xl bg-[#1e1e1e] p-6 text-sm text-zinc-500">正在讀取同步賽程…</div> : fixtures.length ? <div className="space-y-3">{fixtures.map(item => <article key={item.fixtureId} className="overflow-hidden rounded-2xl border border-white/5 bg-[#1e1e1e] shadow-xl">
        <button className="w-full p-4 text-left" onClick={() => setOpenId(openId === item.fixtureId ? null : item.fixtureId)}>
          ...
          {item.hasPrediction ? <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs"><span className="rounded-lg bg-emerald-400/10 py-2 text-emerald-200">主 {pct(item.homeWin)}</span><span className="rounded-lg bg-amber-300/10 py-2 text-amber-100">和 {pct(item.draw)}</span><span className="rounded-lg bg-rose-400/10 py-2 text-rose-200">客 {pct(item.awayWin)}</span></div> : <div className="mt-4 rounded-lg border border-amber-300/20 bg-amber-300/5 py-2 text-center text-xs text-amber-100">Poisson 預測資料尚未同步</div>}
