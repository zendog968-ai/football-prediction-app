...
type Forecast = {
  ...
  diagnostics: { historical_matches_used: number; home_history_matches_used?: number; away_history_matches_used?: number; league_history_matches_used?: number; latest_historical_match: string; dc_available: boolean };
  selected_features: {
    home_elo_pre: number;
    away_elo_pre: number;
    elo_diff_pre: number;
...
export default function Home() {
  ...
  const [fixtureLeagueFilter, setFixtureLeagueFilter] = useState("全部");
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f6f7f5] text-slate-900 selection:bg-amber-200">
      ...
      <main className="relative mx-auto max-w-7xl px-5 py-8 lg:px-8 lg:py-12">
        ...
        <section className="mt-8 overflow-hidden rounded-[2rem] bg-[#07131f] p-5 text-slate-100 shadow-2xl sm:p-8" data-testid="supabase-cache-board">
          <div className="flex flex-col gap-5 border-b border-white/10 pb-6"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.22em] text-emerald-300"><Activity size={14}/> Supabase cache · Live research board</div><h2 className="mt-2 font-serif text-3xl">賽事研究中心</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">以Supabase同步快取優先呈現，每30秒自動核對。只顯示已保存的研究模型與盤口資料。</p></div><span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-[10px] font-bold text-emerald-200">{supabaseUpcomingQuery.data?.available ? "同步中" : "快取確認中"}</span></div><div className="flex gap-2 overflow-x-auto pb-1">{["全部", "英超", "西甲", "歐霸盃", "日職", "美職"].map(label => <button key={label} onClick={() => setFixtureLeagueFilter(label)} className={`shrink-0 rounded-full px-4 py-2 text-xs font-bold transition ${fixtureLeagueFilter === label ? "bg-emerald-400 text-slate-950" : "bg-white/5 text-slate-300 hover:bg-white/10"}`}>{label}</button>)}</div></div>
          {(() => { const fixtures = (supabaseUpcomingQuery.data?.fixtures || []).filter(item => fixtureLeagueFilter === "全部" || item.leagueName.includes(fixtureLeagueFilter)); const selected = fixtures.find(item => item.fixtureId === selectedFixtureId) || fixtures[0]; return fixtures.length ? <div className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_.65fr]"><div className="space-y-3">{fixtures.map(item => <button key={item.fixtureId} onClick={() => setSelectedFixtureId(item.fixtureId)} className={`w-full rounded-2xl border p-4 text-left transition ${(selected?.fixtureId === item.fixtureId) ? "border-emerald-300 bg-emerald-300/10" : "border-white/10 bg-white/[.035] hover:bg-white/[.07]"}`}><div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-[.16em] text-slate-400"><span>{item.leagueName}</span><span>{timestampLabel(item.eventTime)}</span></div><div className="mt-3 flex items-center justify-between gap-3"><div className="font-semibold">{item.homeTeam}</div><div className="rounded-lg bg-white/10 px-3 py-1 font-serif text-lg">{item.predictedScore || "—"}</div><div className="text-right font-semibold">{item.awayTeam}</div></div><div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs"><span className="rounded-lg bg-emerald-400/10 py-2 text-emerald-200">🟢 {percent(item.homeWin)}</span><span className="rounded-lg bg-amber-300/10 py-2 text-amber-100">🟡 {percent(item.draw)}</span><span className="rounded-lg bg-rose-400/10 py-2 text-rose-200">🔴 {percent(item.awayWin)}</span></div><div className="mt-3 rounded-lg border border-white/10 bg-black/10 px-3 py-2 text-[11px] text-slate-300">歷史樣本：{item.homeSampleMatches ?? "—"} 場 vs {item.awaySampleMatches ?? "—"} 場{item.researchSource?.includes("聯賽平均") ? " · 聯賽平均回退" : ""}</div><div className="mt-3 flex justify-between text-xs text-slate-400"><span>{item.recommendation || "研究傾向待確認"}</span><span>{"⭐".repeat(item.confidence)}</span></div></button>)}</div><aside className="rounded-2xl border border-white/10 bg-white/[.035] p-5">{selected ? <><div className="text-[10px] font-bold uppercase tracking-[.18em] text-emerald-300">Match details</div><h3 className="mt-2 text-lg font-bold">{selected.homeTeam} vs {selected.awayTeam}</h3><p className="mt-1 text-xs text-slate-400">{selected.leagueName} · {timestampLabel(selected.eventTime)}</p><div className="mt-3 rounded-xl border border-amber-200/15 bg-amber-200/[.06] px-3 py-2 text-xs text-amber-100">歷史樣本：主隊 {selected.homeSampleMatches ?? "未提供"} 場 · 客隊 {selected.awaySampleMatches ?? "未提供"} 場 · 聯賽 {selected.leagueSampleMatches ?? "未提供"} 場{selected.researchSource?.includes("聯賽平均") ? " · 已觸發聯賽平均回退" : ""}</div><div className="mt-6 space-y-3">{[["主勝",selected.homeWin,"bg-emerald-400"],["和局",selected.draw,"bg-amber-300"],["客勝",selected.awayWin,"bg-rose-400"]].map(([label,value,color]) => <div key={String(label)}><div className="mb-1 flex justify-between text-xs"><span>{String(label)}</span><span>{percent(value as number)}</span></div><div className="h-2 overflow-hidden rounded-full bg-white/10"><div className={`h-full rounded-full ${String(color)}`} style={{width:`${(value as number)*100}%`}}/></div></div>)}</div><div className="mt-6 rounded-xl border border-white/10 bg-black/20 p-3 text-xs leading-5 text-slate-300"><strong className="text-white">研究摘要</strong><br/>精準比分：{selected.predictedScore || "資料不足"}<br/>訊號：{selected.recommendation || "資料不足"}<br/>盤口：已同步盤口將隨下一次快取更新顯示。</div></> : <p className="text-sm text-slate-400">點選賽事卡片查看研究詳情。</p>}</aside></div> : <p className="mt-6 rounded-2xl border border-dashed border-white/15 p-5 text-sm text-slate-400">{supabaseUpcomingQuery.data?.available ? "未來24小時暫無具完整同步研究資料的賽事。" : supabaseUpcomingQuery.data?.reason || "快取資料確認中。"}</p>})()}
        </section>
