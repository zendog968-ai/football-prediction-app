...
export function localizeLeagueName(englishName: string, country?: string | null, translation?: TraditionalLeagueFields): string {
  ...
  if (direct) {
    ...
    if (original === "Pro League" && country === "United-Arab-Emirates") return "阿聯酋超";
    if (original === "Pro League" && country === "Saudi-Arabia") return "沙特超";
    return direct;
  }
  const localizedCountry = country ? COUNTRY_NAMES[country] : null;
