...
每日工作流程的候選建置、資料品質閘門、特徵重建、模型訓練、煙霧測試或發布任何一環失敗時，會自動在GitHub儲存庫建立一則含執行連結的`[Aurelia Data Alert]` Issue。同一個工作流程run採冪等檢查，最多建立一則Issue；原有不可變Release不會被失敗候選版本取代。這個通知是給專案擁有者的可稽核異常事件，應從Issue連結進入工作流程紀錄並檢閱候選稽核檔案。

工作流程另提供只可人工觸發的`force_failure_drill`布林輸入，用來在建立候選資料及發布資產之前安全停止。此輸入只用於驗證異常Issue是否如預期建立，不能在每日排程中啟用；演練完成後應關閉或標記相應Issue，以避免把演練與真實資料品質問題混淆。

網站主頁亦會讀取已驗證Release的SQLite中繼資料，顯示16個資料範圍各自的已完場資料截止日、樣本數、整體樣本總量、來源最近擷取時間及目前Release版本。卡片只反映已成功發布資產，不把失敗候選資料呈現為模型可用資料。
